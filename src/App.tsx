...
const questions: Question[] = [
  ...
  { id: 11, category: 'ツール・スキル', level: '中級', question: '外部MCPサーバーのツールをHermesに接続する機能は？', choices: ['MCP integration', 'Profiles', 'Insights', 'Checkpoints'], answer: 0, explanation: 'MCP Integrationを使うと、MCPサーバーが提供する外部ツールをHermesのツールとして発見・利用できます。', tip: '接続後は hermes mcp list や hermes mcp test NAME で状態を確認できます。' },
  { id: 12, category: '自動化', level: '中級', question: '複数のHermesプロセスを別々の状態で動かすときに重要な考え方は？', choices: ['同じプロファイルを共有する', 'それぞれ専用のProfileを使う', '全てのログを削除する', '毎回モデルを変更する'], answer: 1, explanation: '複数のエージェントを同時に動かす場合は、各プロセスに専用Profileを割り当てると、設定・メモリ・セッションなどの状態が混ざりません。', tip: '同じHermes homeを複数プロセスで共有しないのが安全です。' },
]
